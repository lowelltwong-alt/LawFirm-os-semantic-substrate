...
def lock_sha(lock: dict[str, Any]) -> str | None:
    for key in ['contract_sha','semantic_substrate_sha','substrate_sha','commit_sha','locked_contract_sha']:
        if isinstance(lock.get(key), str) and lock.get(key):
            return lock[key]
    # nested fallback
...
def git_head(repo_dir: Path) -> str | None:
    out = git_output(repo_dir, ['rev-parse','HEAD'])
    return out[0] if out else None

def main() -> int:
    ap = argparse.ArgumentParser(description='Workspace-level contract lock drift check.')
    ap.add_argument('--workspace', default='.')
    ap.add_argument('--allow-documented-rationale', action='store_true', help='Allow old pins only if pin_rationale or lock_rationale is present')
    args = ap.parse_args()
    workspace = Path(args.workspace).resolve()
    repos = discover_repo_dirs(workspace)
    substrate = repos.get('LawFirm-os-semantic-substrate')
    if not substrate:
        print('Missing Semantic Substrate repo', file=sys.stderr); return 1
    head = git_head(substrate)
    errors=[]
    for repo, repo_dir in sorted(repos.items()):
        lock_path = repo_dir / 'contracts.lock.json'
        ...
        if not sha or not head:
            continue
        if sha != head:
            rationale = lock.get('pin_rationale') or lock.get('lock_rationale') or lock.get('human_review_rationale')
            if args.allow_documented_rationale and rationale:
                continue
            errors.append(f'{repo}: contracts.lock.json pins {sha}, Semantic Substrate HEAD is {head}; bump or document human-approved rationale')
    if errors:
