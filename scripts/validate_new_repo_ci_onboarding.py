...
def main() -> int:
    ap = argparse.ArgumentParser(description='Validate that every LawFirm OS repo is registered and has CI test routing metadata.')
    ...
    for repo, repo_dir in sorted(repos.items()):
        if repo not in active:
            errors.append(f'{repo}: repo directory exists but is not active in lawfirm-os-repo-registry.json')
        for required in ['README.md','AGENTS.md','AI_WORK_START_HERE.md','skill-agent-manifest.json','ci-test-manifest.json']:
            if not (repo_dir / required).exists():
                errors.append(f'{repo}: missing required onboarding file {required}')
    for repo in sorted(active):
        aliases = [repo, repo + '-main'] + KNOWN_REPO_ALIASES.get(repo, [])
        if not any((workspace / alias).exists() for alias in aliases):
            errors.append(f'{repo}: active in registry but no matching workspace directory found')
    if errors:
        print('New repo CI onboarding validation failed:', file=sys.stderr)
        for e in errors:
            print(f'- {e}', file=sys.stderr)
        return 1
    print('New repo CI onboarding validation passed.')
    return 0
